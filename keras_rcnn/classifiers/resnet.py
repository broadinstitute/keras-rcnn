...
def residual(classes, mask=False, features=512):
    ...
    def f(x):
        ...
        y = keras_resnet.blocks.time_distributed_bottleneck_2d(features,
                                                               stage=3,
                                                               block=0,
                                                               stride=1)(x)
        y = keras_resnet.blocks.time_distributed_bottleneck_2d(features,
        ...
        y = keras.layers.TimeDistributed(
            keras.layers.GlobalAveragePooling2D())(y)

        score = keras.layers.TimeDistributed(
            keras.layers.Dense(classes, activation="softmax"))(y)
        boxes = keras.layers.TimeDistributed(keras.layers.Dense(4 * classes))(
            y)
