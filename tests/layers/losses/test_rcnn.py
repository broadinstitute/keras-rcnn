...
class TestRCNN():
    def test_call(self):
        ...
        target_scores = keras.backend.variable([[0, 0, 1], [0, 1, 0], [0, 1, 0]])
        target_scores = keras.backend.expand_dims(target_scores, 0)
        output_scores = keras.backend.variable([[0, 0.9, 0.1], [0.2, 0.8, 0], [0.1, 1e-4, 0.9-1e-4]])
        output_scores = keras.backend.expand_dims(output_scores, 0)


        target_deltas = keras.backend.variable([[0, 0, 0, 0, 0, 0, 0, 0, -.1, .2, .3, .4], [-.1, .2, 1, -3, 0, -1, 1.3, -.1, 2, .3, -1.5, .6], [-.1, .2, 1, -3, 0, -1, 1.3, -.1, 2, .3, -1.5, .6]])
        target_deltas = keras.backend.expand_dims(target_deltas, 0)
        output_deltas = target_deltas

        weights = keras.backend.dot(target_scores[0], keras.backend.reshape(keras.backend.sum(target_scores, axis=1), (-1, 1)))
        ...
        layer.call([target_deltas, target_scores, output_deltas, output_scores], balanced=weights)

        classification_loss = layer.classification_loss()

        classification_loss = keras.backend.eval(classification_loss)

        numpy.testing.assert_almost_equal(classification_loss, 2.33978, decimal=4)
